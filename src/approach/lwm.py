...
class Appr(Inc_Learning_Appr):
    ...
    def _save_gradcam_examples(self, t, trn_loader):
        ...
        output_orig_filename = os.path.join(
            self.logger.exp_path, "{t}_orig_post_train.png".format(t=t)
        )
        output_map_filename = os.path.join(
            self.logger.exp_path, "{t}_gcam_post_train.png".format(t=t)
        )
        print(
            "Task {} - Saving {} samples to: {}".format(
                t, self.log_gradcam_samples, output_orig_filename
            )
        )
        save_image(
            torch.cat(self._samples_to_log_X),
            output_orig_filename,
            normalize=True,
            nrow=(t + 1),
        )
        print(
            "Task {} - Saving {} samples with heatmaps to: {}".format(
                t, self.log_gradcam_samples, output_map_filename
            )
        )
        X_with_gradcam = []
    ...
    def train_epoch(self, t, trn_loader):
        ...
        with GradCAM(self.model_old, self.gradcam_layer) as gradcam_old:
            for images, targets in trn_loader:
                ...
                loss = self.criterion(
                    t, outputs, targets.to(self.device), outputs_old, attmap, attmap_old
                )
                # Backward
    ...
    def eval(self, t, val_loader):
        ...
        self.model.eval()
        with GradCAM(self.model, self.gradcam_layer) as gradcam, GradCAM(
            self.model_old, self.gradcam_layer
        ) as gradcam_old:
    ...
    def attention_distillation_loss(self, attention_map1, attention_map2):
        ...
        attention_map1 = torch.nn.functional.normalize(
            attention_map1.view(attention_map1.size(0), -1),
            p=2,
            dim=1,
            eps=1e-12,
            out=None,
        )
        attention_map2 = torch.nn.functional.normalize(
            attention_map2.view(attention_map1.size(0), -1),
            p=2,
            dim=1,
            eps=1e-12,
            out=None,
    ...
    def criterion(
        self, t, outputs, targets, outputs_old=None, attmap=None, attmap_old=None
    ):
        """Returns the loss value"""
        loss = 0
        if t > 0 and outputs_old is not None:
            # Knowledge distillation loss for all previous tasks
            loss += self.beta * self.cross_entropy(
...
class GradCAM:
    ...
    def __init__(self, model: LLL_Net, layer_name, retain_graph=False):
        self.model = model
        assert hasattr(
            self.model.model, layer_name
        ), "Model {} has got attribute layer: {}".format(type(self.model), layer_name)
