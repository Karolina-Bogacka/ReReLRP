...
from datasets_local.memory_dataset import MemoryDataset
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform):
        if self._is_active():
            self.images, self.labels = self.exemplars_selector(
                model, trn_loader, selection_transform
            )
