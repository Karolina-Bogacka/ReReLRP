...
class GridSearch:
    ...
    def __init__(
        ...
        appr_ft,
        seed,
        gs_config="gridsearch_config",
        acc_drop_thr=0.2,
        hparam_decay=0.5,
        max_num_searches=7,
    ):
        self.seed = seed
        GridSearchConfig = getattr(
            importlib.import_module(name=gs_config), "GridSearchConfig"
        )
    ...
    def search_lr(self, model, t, trn_loader, val_loader, appr=None):
        ...
        if t > 0:
            ...
            list_lr = [lr for lr in gen_params["lr"] if lr < self.lr_first][
                : gen_params["lr_searches"][0]
            ]
        else:
        ...
        self.appr_ft.model = model
        # Iterate through the other variable parameters
        for curr_lr in list_lr:
            utils.seed_everything(seed=self.seed)
            #self.appr_ft.model.load_state_dict(copy.deepcopy(model.state_dict()))
            if type(appr) == approach.rere_lrp.Appr:
                self.appr_ft.model = appr.prepare_grid(t)
                self.appr_ft.ex_flag = False
            else:
                self.appr_ft.model = model
            self.appr_ft.nepochs = 2
            ...
            with torch.no_grad():
                self.appr_ft.model.load_state_dict(copy.deepcopy(init_state))
        ...
        if tradeoff_name is not None and t > 0:
            ...
            while num_searches < self.max_num_searches:
                ...
                appr_gs = type(appr)(
                    deepcopy(appr.model),
                    appr.device,
                    exemplars_dataset=appr.exemplars_dataset,
                )
