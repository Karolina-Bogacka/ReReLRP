...
"""
Initial code taken from https://github.com/galidor/PyTorchPartialLayerFreezing/blob/main/partial_freezing.py
Credit goes to Mikolaj Jankowski as detailed in LICENSE file in this directory
"""


def freeze_conv1d_params(
    layer,
    weight_indices,
    bias_indices=None,
    weight_hook_handle=None,
    ...
    if (weight_indices == [] or weight_indices is None) and (
        bias_indices == [] or bias_indices is None
    ):
        return
    ...
    weight_multiplier[weight_indices] = weight_multi_val
    weight_multiplier = weight_multiplier.view(-1, 1, 1)
    freezing_hook_weight = lambda grad: freezing_hook_weight_full(
    ...
    if layer.bias is not None:
        ...
        bias_multiplier[bias_indices] = bias_multi_val
...
def freeze_conv2d_params(
    ...
    if layer.bias is not None and bias_indices is not None:
        if max(bias_indices) >= layer.bias.shape[0]:
            raise IndexError(
                "bias_indices must be less than the number output channels"
            )
...
if __name__ == "__main__":
    # visualize_experiments(rnf=False)
    freeze_linear_params(2, [], [])
