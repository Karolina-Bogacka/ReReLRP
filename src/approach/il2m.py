...
class Appr(Inc_Learning_Appr):
    ...
    def il2m(self, t, trn_loader):
        ...
        for cls in range(
            old_classes_number, old_classes_number + self.model.task_cls[t]
        ):
        ...
        with torch.no_grad():
            ...
            for images, targets in trn_loader:
                ...
                scores = np.array(
                    torch.cat(outputs, dim=1).data.cpu().numpy(), dtype=np.float
                )
    ...
    def calculate_metrics(self, outputs, targets):
        ...
        if self.ft_train:
            ...
            for m in range(len(pred)):
                ...
                pred[m] = (
                    outputs[this_task][m].argmax() + self.model.task_offset[this_task]
                )
                ...
                if old_classes_number:
                    ...
                    if int(pred[m]) >= old_classes_number:
                        for o in range(old_classes_number):
                            ...
                            rectified_outputs[m, o] *= (
                                self.init_classes_means[o]
                                / self.current_classes_means[o]
                            ) * (
                                self.models_confidence[-1]
                                / self.models_confidence[o_task]
                            )
                        pred[m] = rectified_outputs[m].argmax()
