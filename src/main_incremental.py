...
import torch
import utils
from datasets_local.data_loader import get_loaders
from datasets_local.dataset_config import dataset_config
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        if t < args.gridsearch_tasks:
            ...
            best_ft_acc, best_ft_lr = gridsearch.search_lr(
                appr.model, t, trn_loader[t], val_loader[t], appr=appr
            )
            # Apply to approach
            appr.lr = best_ft_lr
        ...
        for u in range(t + 1):
            ...
            logger.log_scalar(
                task=t, iter=u, name="loss", group="test", value=test_loss
            )
            logger.log_scalar(
                task=t, iter=u, name="acc_taw", group="test", value=100 * acc_taw[t, u]
            )
            logger.log_scalar(
                task=t, iter=u, name="acc_tag", group="test", value=100 * acc_tag[t, u]
            )
            logger.log_scalar(
                task=t,
                iter=u,
        ...
        logger.log_result(
            acc_taw.sum(1) / np.tril(np.ones(acc_taw.shape[0])).sum(1),
            name="avg_accs_taw",
            step=t,
        )
        logger.log_result(
            acc_tag.sum(1) / np.tril(np.ones(acc_tag.shape[0])).sum(1),
            name="avg_accs_tag",
            step=t,
        )
        aux = np.tril(
        ...
        logger.log_result(
            (acc_taw * aux).sum(1) / aux.sum(1), name="wavg_accs_taw", step=t
        )
        logger.log_result(
            (acc_tag * aux).sum(1) / aux.sum(1), name="wavg_accs_tag", step=t
        )
