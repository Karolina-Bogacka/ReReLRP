...
class Appr(Inc_Learning_Appr):
    ...
    def post_train_process(self, t, trn_loader):
        ...
        with torch.no_grad():
            curr_params = {
                n: p for n, p in self.model.model.named_parameters() if p.requires_grad
            }
            for n, p in self.scores.items():
                curr_score[n] = self.w[n] / (
                    self.fisher[n] * ((curr_params[n] - self.older_params[n]) ** 2)
                    + self.damping
                )
                self.w[n].zero_()
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            loss = torch.nn.functional.cross_entropy(
                torch.cat(outputs, dim=1), targets.to(self.device)
            )
            self.optimizer.zero_grad()
        ...
        if t > 0:
            ...
            for n, p in self.model.model.named_parameters():
                if n in self.fisher.keys():
                    loss_reg += torch.sum(
                        (self.fisher[n] + self.scores[n])
                        * (p - self.older_params[n]).pow(2)
                    )
