...
from torchvision.datasets import EMNIST as TorchVisionEMNIST
...
def get_loaders(
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        # datasets
        trn_dset, val_dset, tst_dset, curtaskcla = get_datasets(
            cur_dataset,
            dc["path"],
            num_tasks,
            nc_first_task,
...
def get_datasets(
    ...
    if "fashionmnist" in dataset:
        ...
        tvmnist_trn = TorchVisionEMNIST(
            path, split="letters", train=True, download=True
        )
        tvmnist_tst = TorchVisionEMNIST(
            path, split="letters", train=False, download=True
        )
        trn_data = {"x": tvmnist_trn.data.numpy(), "y": tvmnist_trn.targets.tolist()}
        tst_data = {"x": tvmnist_tst.data.numpy(), "y": tvmnist_tst.targets.tolist()}
        # compute splits
        ...
        tvcifar_trn = load_dataset("zh-plus/tiny-imagenet", split="train")
        tvcifar_tst = load_dataset("zh-plus/tiny-imagenet", split="valid")
        trn_data = {"x": tvcifar_trn.data, "y": tvcifar_trn.targets}
        tst_data = {"x": tvcifar_tst.data, "y": tvcifar_tst.targets}
        # compute splits
        all_data, taskcla, class_indices = memd.get_data(
        ...
        all_data, taskcla, class_indices = basedat.get_data(
            path,
            num_tasks=num_tasks,
            nc_first_task=nc_first_task,
            validation=validation,
...
def get_transforms(resize, pad, crop, flip, normalize, extend_channel):
    ...
    if extend_channel is not None:
        trn_transform_list.append(
            transforms.Lambda(lambda x: x.repeat(extend_channel, 1, 1))
        )
        tst_transform_list.append(
            transforms.Lambda(lambda x: x.repeat(extend_channel, 1, 1))
        )

    return transforms.Compose(trn_transform_list), transforms.Compose(
        tst_transform_list
    )
