...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader, transform):
        ...
        with override_dataset_transform(
            trn_loader.dataset, transform
        ) as ds_for_selection:
            ...
            selected_indices = self._select_indices(
                model, sel_loader, exemplars_per_class, transform
            )
        with override_dataset_transform(
            trn_loader.dataset, Lambda(lambda x: np.array(x))
        ) as ds_for_raw:
            x, y = zip(*(ds_for_raw[idx] for idx in selected_indices))
        clock1 = time.time()
        print(
            "| Selected {:d} train exemplars, time={:5.1f}s".format(
                len(x), clock1 - clock0
            )
        )
        return x, y
...
class RandomExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(
        ...
        num_start = sum(model.task_cls[:-1])
        result = []
        labels = self._get_labels(sel_loader)  # - int(model.task_offset[-1])
        print(labels)
        for curr_cls in range(num_start, num_cls):
    ...
    def _get_labels(self, sel_loader):
        if hasattr(sel_loader.dataset, "labels"):  # BaseDataset, MemoryDataset
            ...
            raise RuntimeError(
                "Unsupported dataset: {}".format(sel_loader.dataset.__class__.__name__)
            )
        return labels
...
class HerdingExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(
        ...
        model_device = next(
            model.parameters()
        ).device  # we assume here that whole model is on a single device
        ...
        for curr_cls in np.unique(extracted_targets):
            ...
            assert exemplars_per_class <= len(
                cls_ind
            ), f"Not enough samples to store {exemplars_per_class} {len(cls_ind)}"
            ...
            for k in range(exemplars_per_class):
                ...
                for item in cls_ind:
                    if item not in selected:
                        ...
                        if newone is None or newonefeat is None:
                            newone = item
                            newonefeat = feat
                            dist_min = dist
                        if dist < dist_min:
                            ...
                            newone = item
                            newonefeat = feat
                if newone is not None and newonefeat is not None:
...
class EntropyExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(
        ...
        with torch.no_grad():
            model.eval()
            for images, targets in sel_loader:
                extracted_logits.append(
                    torch.cat(model(images.to(model_device)), dim=1)
                )
                extracted_targets.extend(targets)
        ...
        for curr_cls in np.unique(extracted_targets):
            ...
            minus_entropy = (probs * log_probs).sum(
                1
            )  # change sign of this variable for inverse order
